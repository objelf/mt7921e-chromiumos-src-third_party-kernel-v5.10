...
static int mt8195_afe_config_cm(struct mtk_base_afe *afe,
	const struct mt8195_afe_channel_merge *cm, unsigned int channels)
{
	if (cm == NULL)
		return -EINVAL;
...
static int mt8195_afe_enable_cm(struct mtk_base_afe *afe,
	const struct mt8195_afe_channel_merge *cm, bool enable)
{
	if (cm == NULL)
		return -EINVAL;
	...
	int enable)
{
	...
	if (enable) {
		mt8195_afe_prepare_clk(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL10]);
		mt8195_afe_prepare_clk(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL8]);
	} else {
		mt8195_afe_unprepare_clk(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL8]);
		mt8195_afe_unprepare_clk(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL10]);
	}
	...
	int enable)
{
	...
	if (enable) {
		/* DL8_DL10_MEM */
		mt8195_afe_clk_enable(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL10]);
		udelay(1);
		/* DL8_DL10_AGENT */
		mt8195_afe_clk_enable(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL8]);
	} else {
		/* DL8_DL10_AGENT */
		mt8195_afe_clk_disable(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL8]);
		/* DL8_DL10_MEM */
		mt8195_afe_clk_disable(afe, afe_priv->clk[MT8195_CLK_AUD_MEMIF_DL10]);
	}
	...
	struct snd_soc_dai *dai)
{
	...
	if (data->ch_num_reg >= 0) {
		regmap_update_bits(afe->regmap, data->ch_num_reg,
			...
			params_channels(params) << data->ch_num_shift);
	}
...
static int mt8195_memif_1x_en_sel_put(
...
static int mt8195_asys_irq_1x_en_sel_put(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
...
static int mt8195_afe_suspend(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);
...
static int mt8195_afe_resume(struct device *dev)
{
	dev_dbg(dev, "%s\n", __func__);
...
static const struct dev_pm_ops mt8195_afe_pm_ops = {
	...
	SET_SYSTEM_SLEEP_PM_OPS(mt8195_afe_suspend, mt8195_afe_resume)
};
