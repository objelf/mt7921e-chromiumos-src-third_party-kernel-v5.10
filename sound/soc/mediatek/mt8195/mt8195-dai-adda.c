				   ...
				   int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt8195_afe_set_clk_parent(afe,
			afe_priv->clk[MT8195_CLK_TOP_AUDIO_H_SEL],
			afe_priv->clk[MT8195_CLK_TOP_APLL1]);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt8195_afe_set_clk_parent(afe,
			afe_priv->clk[MT8195_CLK_TOP_AUDIO_H_SEL],
			afe_priv->clk[MT8195_CLK_XTAL_26M]);
		break;
	default:
		break;
	}
...
static const struct snd_soc_dapm_widget mtk_dai_adda_widgets[] = {
	...
	SND_SOC_DAPM_MIXER("O176", SND_SOC_NOPM, 0, 0,
		mtk_dai_adda_o176_mix, ARRAY_SIZE(mtk_dai_adda_o176_mix)),
	SND_SOC_DAPM_MIXER("O177", SND_SOC_NOPM, 0, 0,
		mtk_dai_adda_o177_mix, ARRAY_SIZE(mtk_dai_adda_o177_mix)),
	...
	struct snd_soc_dai *dai)
{
	...
	mt8195_afe_enable_clk(afe,
		afe_priv->clk[MT8195_CLK_INFRA_AO_AUDIO_26M_B]);

	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		mt8195_afe_enable_clk(afe,
			afe_priv->clk[MT8195_CLK_AUD_DAC]);

	} else {
		if (dai->id == MT8195_AFE_IO_UL_SRC1)
			mt8195_afe_enable_clk(afe,
				afe_priv->clk[MT8195_CLK_AUD_ADC]);
		else if (dai->id == MT8195_AFE_IO_UL_SRC2)
			mt8195_afe_enable_clk(afe,
				afe_priv->clk[MT8195_CLK_AUD_ADDA6_ADC]);
	}
	return 0;
}
